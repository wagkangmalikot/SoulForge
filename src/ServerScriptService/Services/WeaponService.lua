...
-- ============================================================================
-- 4. EQUIPMENT MODEL DISPATCHERS & VISUAL ATTACHMENTS
-- ============================================================================

local function createSwordModel(weaponId: string?): (Model, BasePart, Trail)
	if weaponId == "RockhideFang" or weaponId == "Rockhide" then
...
local function createShieldModel(armsId: string?): (Model, BasePart)
...
function WeaponService.EquipWeapons(character: Model, weaponSetId: string?, shieldSetId: string?)
	...
	local targetWeapon = weaponSetId or equipped.Weapon or (charData and charData.EquippedWeapon) or "StandardSword"
	local targetArms = shieldSetId or equipped.Arms or "StandardArms"
	local targetHead = equipped.Head or "StandardHelm"
	local targetBody = equipped.Body or "StandardChest"
	local targetFeet = equipped.Feet or "StandardFeet"
	...
	-- 1. Equip Sword
	local swordModel, swordHandle = createSwordModel(targetWeapon)
	local swordGrip = Instance.new("Motor6D")
	swordGrip.Name = "SwordGrip"
	swordGrip.Part0 = rightHand
	swordGrip.Part1 = swordHandle
	swordGrip.C0 = CFrame.new(0, -0.4, -0.2) * CFrame.Angles(math.rad(-90), math.rad(0), math.rad(0))
	swordGrip.Parent = rightHand
	swordModel.Parent = character

	-- 2. Equip Shield
	local shieldModel, shieldHandle = createShieldModel(targetArms)
