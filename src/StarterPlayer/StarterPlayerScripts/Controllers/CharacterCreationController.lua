...
function CharacterCreationController.Start()
	-- Run on its own thread so a dungeon server's 5-second timeout below
	-- doesn't delay Main.client.lua's synchronous calls into the controllers
	-- that run after this one. This whole flow is hub-only: a dungeon server
	-- never places RockhidePortal, the same signal DungeonPortalController
	-- already uses to detect this, reused here so the intro card never
	-- flashes on a dungeon server.
	task.spawn(function()
		local portalPart = workspace:WaitForChild("RockhidePortal", 5)
		if not portalPart then
			-- Dungeon server -- no character-entry flow here.
			return
		end
