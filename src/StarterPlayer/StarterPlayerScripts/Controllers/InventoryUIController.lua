...
local function renderDetailView(itemId: string?)
	...
	-- Item Artwork Tile
	local detailIcon = ItemIconHelper.CreateItemIcon(hdr, itemId, UDim2.new(0, 52, 0, 52), true)
	detailIcon.Position = UDim2.new(0, 12, 0.5, -26)
	...
	slotTag.Position = UDim2.new(0, 72, 0, 6)
	slotTag.Size = UDim2.new(0.6, 0, 0, 16)
	slotTag.Parent = hdr
	...
	sub.Position = UDim2.new(0, 72, 0, 46)
	sub.Size = UDim2.new(0.6, 0, 0, 16)
	sub.Parent = hdr

	-- Status Badge
	local statusBadge = Instance.new("Frame")
	statusBadge.Size = UDim2.new(0, 88, 0, 24)
	statusBadge.Position = UDim2.new(1, -98, 0, 10)
	...
	detailsScroll.Size = UDim2.new(1, 0, 1, -120)
	detailsScroll.Position = UDim2.new(0, 0, 0, 74)
...
local function renderLoadoutSlots()
	...
	for orderIndex, slotName in ipairs(SLOT_ORDER) do
		...
		local iconTile = equippedItemId and ItemIconHelper.CreateItemIcon(card, equippedItemId, UDim2.new(0, 32, 0, 32), true) or ItemIconHelper.CreateSlotIcon(card, slotName, UDim2.new(0, 32, 0, 32), false)
		iconTile.Position = UDim2.new(0, 8, 0.5, -16)

		local slotTitle = Instance.new("TextLabel")
		slotTitle.Text = string.upper(slotName)
		slotTitle.TextColor3 = THEME.amberBright
		slotTitle.TextSize = 9
		slotTitle.Font = Enum.Font.GothamBold
		slotTitle.BackgroundTransparency = 1
		slotTitle.TextXAlignment = Enum.TextXAlignment.Left
		slotTitle.Position = UDim2.new(0, 48, 0, 6)
		slotTitle.Size = UDim2.new(1, -120, 0, 14)
		...
		itemNameLabel.Position = UDim2.new(0, 48, 0, 22)
		itemNameLabel.Size = UDim2.new(1, -120, 0, 18)
		itemNameLabel.Parent = card

		if item then
			local eqBadge = Instance.new("Frame")
			eqBadge.Size = UDim2.new(0, 62, 0, 20)
			eqBadge.Position = UDim2.new(1, -68, 0.5, -10)
	...
	-- Update Set Bonus
	local rockhideEquipped, totalRockhide = getEquippedCountForSet("Rockhide")
	local isActive = (rockhideEquipped == totalRockhide and totalRockhide > 0)

	if setBonusPips then
		setBonusPips:ClearAllChildren()
		for i = 1, totalRockhide do
			local pieceId = EquipmentData.Sets["Rockhide"].pieces[i]
			local isPieceOn = isEquipped(pieceId)
			local pip = Instance.new("Frame")
			pip.Size = UDim2.new(0, 16, 0, 16)
			pip.BackgroundColor3 = isPieceOn and THEME.amberBright or THEME.panelBg
			pip.BorderSizePixel = 0
			makeCorner(pip, 3)
			makeStroke(pip, isPieceOn and THEME.borderBright or THEME.borderDim, 1)
	...
	if setBonusText then
		if isActive then
			setBonusText.Text = string.format("SEISMIC FURY ACTIVE (%d/%d): +15%% damage when below 50%% HP", rockhideEquipped, totalRockhide)
			setBonusText.TextColor3 = THEME.amberBright
		else
			setBonusText.Text = string.format("Rockhide Set (%d/%d): Seismic Fury (Equip all 5 pieces)", rockhideEquipped, totalRockhide)
...
local function renderItemsList()
	...
	if activeTab == "Equipment" then
		...
		for orderIndex, entry in ipairs(ownedList) do
			...
			local iconTile = ItemIconHelper.CreateItemIcon(card, itemId, UDim2.new(0, 32, 0, 32), isSelected)
			iconTile.Position = UDim2.new(0, 8, 0.5, -16)
	...
	elseif activeTab == "Materials" then
		...
		for matId, matDef in pairs(EquipmentData.Materials) do
			...
			local iconTile = ItemIconHelper.CreateMaterialIcon(card, matId, UDim2.new(0, 32, 0, 32))
			iconTile.Position = UDim2.new(0, 8, 0.5, -16)
			...
			descLabel.Position = UDim2.new(0, 48, 0, 24)
			descLabel.Size = UDim2.new(1, -130, 0, 16)
			descLabel.Parent = card

			-- Count pill
			local countPill = Instance.new("Frame")
			countPill.Size = UDim2.new(0, 72, 0, 22)
			countPill.Position = UDim2.new(1, -78, 0.5, -11)
...
local function buildUI()
	...
	-- Main Modal Frame (880 × 580)
	mainModal = Instance.new("Frame")
	mainModal.Name = "MainModal"
	mainModal.Size = UDim2.new(0, 880, 0, 580)
	mainModal.Position = UDim2.new(0.5, -440, 0.5, -290)
	...
	headerTitle.Size = UDim2.new(0.55, 0, 0, 20)
	...
	headerSubtitle.Position = UDim2.new(0, 16, 0, 26)
	headerSubtitle.Size = UDim2.new(0.55, 0, 0, 16)
	headerSubtitle.Parent = header

	-- Gold Pill
	local goldPill = Instance.new("Frame")
	goldPill.Name = "GoldPill"
	goldPill.Size = UDim2.new(0, 150, 0, 28)
	goldPill.Position = UDim2.new(1, -204, 0.5, -14)
	...
	local coinIcon = ItemIconHelper.CreateGoldCoinIcon(goldPill, UDim2.new(0, 18, 0, 18))
	coinIcon.Position = UDim2.new(0, 8, 0.5, -9)
	...
	goldLabel.Position = UDim2.new(0, 32, 0, 0)
	goldLabel.Size = UDim2.new(1, -36, 1, 0)
	...
	-- Left Column: Equipped Loadout + Set Bonus (240px wide)
	...
	loadoutSlotsContainer.Size = UDim2.new(1, -12, 0, 260)
	...
	setBonusCard.Size = UDim2.new(1, -12, 1, -308)
	setBonusCard.Position = UDim2.new(0, 6, 0, 302)
	...
	setBonusPips.Size = UDim2.new(1, 0, 0, 20)
	setBonusPips.Position = UDim2.new(0, 0, 1, -22)
	...
	-- Middle Column: Bag Items / Materials Pouch (300px wide)
	local midCol = Instance.new("Frame")
	midCol.Name = "MiddleBag"
	midCol.Size = UDim2.new(0, 300, 1, 0)
	midCol.Position = UDim2.new(0, 248, 0, 0)
	...
	tabsHeader.Size = UDim2.new(1, 0, 0, 32)
	...
	itemsScroll.Size = UDim2.new(1, -12, 1, -40)
	itemsScroll.Position = UDim2.new(0, 6, 0, 36)
	...
	rightCol.Size = UDim2.new(1, -556, 1, 0)
	rightCol.Position = UDim2.new(0, 556, 0, 0)
	...
	detailViewContainer.Size = UDim2.new(1, 0, 1, -48)
	...
	equipActionButton.Size = UDim2.new(1, 0, 0, 38)
	equipActionButton.Position = UDim2.new(0, 0, 1, -40)
	equipActionButton.BackgroundColor3 = THEME.greenDark
...
function InventoryUIController.Open()
	...
	mainModal.Position = UDim2.new(0.5, -440, 0.5, -265)
	...
	TweenService:Create(mainModal, TweenInfo.new(0.24, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
		Position = UDim2.new(0.5, -440, 0.5, -290),
	...
	-- If no item is selected, select the first owned piece
